 ...
 * @namespace rules
 */
var GENERATED_RULE_ITEM_TAG = "GENERATED_RULE_ITEM";
var items = require('openhab/items');
var utils = require('openhab/utils');
var log = require('openhab/log')('rules');
var osgi = require('openhab/osgi');
var triggers = require('openhab/triggers');
var automationManager = require('@runtime/RuleSupport').automationManager;
var RuleManager = osgi.getService("org.openhab.core.automation.RuleManager");
var factory = require('@runtime/rules').factory;
/**
 ...
 */
var itemNameForRule = function (ruleConfig) {
 ...
 */
var linkItemToRule = function (rule, item) {
    JSRule({
 ...
 */
var getGroupsForItem = function (ruleConfig) {
 ...
 */
var JSRule = function (ruleConfig) {
    var ruid = ruleConfig.id || ruleConfig.name.replace(/[^\w]/g, "-") + "-" + utils.randomUUID();
    var ruTemplateid = ruleConfig.name.replace(/[^\w]/g, "-") + "-" + utils.randomUUID();
    log.info("Adding rule: {}", ruleConfig.name ? ruleConfig.name : ruid);
    var SimpleRule = Java.extend(Java.type('org.openhab.core.automation.module.script.rulesupport.shared.simple.SimpleRule'));
    var doExecute = function (module, input) {
        try {
            return ruleConfig.execute(getTriggeredData(input));
        }
        catch (error) {
            log.error("Failed to execute rule ".concat(ruid, ": ").concat(error, ": ").concat(error.stack));
            throw error;
        }
    };
    var rule = new SimpleRule({
        execute: doExecute,
        getUID: function () { return ruid; }
    });
...
var currentProvider = automationManager;
var withNewRuleProvider = function (fn) {
    var cachedRules = [];
    currentProvider = {
        addRule: function (r) {
    ...
    try {
        fn();
        var provider = factory.newRuleProvider(cachedRules);
...
var withManagedProvider = function (fn) {
 ...
 * @returns {HostRule} the created rule
 */
var SwitchableJSRule = function (ruleConfig) {
    if (!ruleConfig.name) {
    ...
    if (item.isUninitialized) {
        ...
        var historicState = item.history.latestState();
...
var getTriggeredData = function (input) {
    var event = input.get('event');
...
module.exports = {
    withNewRuleProvider: withNewRuleProvider,
    JSRule: JSRule,
    SwitchableJSRule: SwitchableJSRule
};
